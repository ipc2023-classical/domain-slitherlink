...
main = do
     ...
     gen <- newStdGen
     case args of
          [nr, nc, fnprob, fnsol] -> do
               let (problem, solution) = generate (read nr) (read nc) gen
               writeFile fnprob $ showProblem problem
               writeFile fnsol $ showState solution
          _ -> error "usage: generate rows columns problem.txt solution.txt"
